...
namespace NetworkPr_Server
{
    internal class Program
    {
        ...
        private static int MaxQuotesPerClient = 5;
        private static Dictionary<TcpClient, int> clientQuoteCount = new Dictionary<TcpClient, int>();
        ...
        static async Task Main()
        {
            ...
            while (true)
            {
                if (clients.Count < MaxConnectedClients) // Check if maximum clients reached
                {
                    TcpClient client = await server.AcceptTcpClientAsync();
                    clients.Add(client);
                    clientQuoteCount[client] = 0; // Initialize the quote count for this client
                    LogConnection(client);
                    _ = Task.Run(() => HandleClient(client));
                }
                else
                {
                    Console.WriteLine("Maximum clients reached. Please try again later.");
                }
            }
        }
        ...
        static async Task HandleClient(TcpClient client)
        {
            ...
            try
            {
                while (true)
                {
                    ...
                    {
                        if (clientQuoteCount[client] < MaxQuotesPerClient)
                        {
                            string randomQuote = GetRandomQuote();
                            await writer.WriteLineAsync(randomQuote);
                            clientQuoteCount[client]++;
                        }
                        else
                        {
                            await writer.WriteLineAsync("MAX_QUOTES_REACHED");
                        }
                    }
                }
            }
            catch (IOException)
            {
                ...
                clientQuoteCount.Remove(client);
